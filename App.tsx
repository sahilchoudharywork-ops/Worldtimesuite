...
const toShortCityPath = (fromSlug: string, toSlug: string) => `/${fromSlug}-to-${toSlug}`;
...
const App: React.FC = () => {
  ...
  // Safety-net only: if legacy URL reaches SPA, normalize without full reload.
  useEffect(() => {
    if (!cityRoute) return;
    if (!isLegacyTimezoneCityRoute(window.location.pathname)) return;

    const canonical = toShortCityPath(cityRoute.fromSlug, cityRoute.toSlug);
    if (normalizePath(window.location.pathname) !== canonical) {
      window.history.replaceState({}, '', canonical);
    }
  }, [cityRoute]);
  ...
  // Keep static sections synced. Do not override dynamic city routes.
  useEffect(() => {
    const path = normalizePath(window.location.pathname);
    const isDynamicCityRoute = !!parseCityRoute(path);
  ...
  // Single canonical source: always short URL for city pages.
  useEffect(() => {
    const seo = seoByPage[currentPage] || seoByPage[Page.CONVERTER];
    const origin = typeof window !== 'undefined' ? window.location.origin : 'https://worldtimesuite.com';

    const canonicalPath = cityRoute
      ? toShortCityPath(cityRoute.fromSlug, cityRoute.toSlug)
      : normalizePath(window.location.pathname);
