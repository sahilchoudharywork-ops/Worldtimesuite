...
const Calendar: React.FC<CalendarProps> = ({ isDark }) => {
  ...
  const defaultTasks: Task[] = [
    {
      ...
      dueDate: new Date(2026, 1, 8).toISOString().split('T')[0],
      time: '14:00',
      priority: 'high',
      completed: false,
      repeat: 'none'
    },
    {
      ...
      dueDate: new Date(2026, 1, 12).toISOString().split('T')[0],
      time: '10:00',
      priority: 'medium',
      completed: true,
      repeat: 'none'
  ...
  const handleConfirmAction = () => {
    ...
    if (mode === 'delete' && taskId) {
      setTasks(prev => prev.filter(t => t.id !== taskId));
      if (previewTask?.id === taskId) closeTaskPreview();
    } else if (mode === 'save') {
      setTasks(prev =>
        prev.map(t =>
          t.id === editingTaskId
  ...
  return (
    <div className={`flex flex-col xl:flex-row min-h-[700px] font-['Helvetica Neue',Helvetica,sans-serif] ${bgColor} ${textColor} transition-colors duration-500 relative`}>
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-xl bg-black/40">
          <div className={`${modalBg} w-full max-w-lg rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 shadow-2xl border ${borderColor} animate-in fade-in zoom-in duration-300`}>
            <div className="flex justify-between items-start mb-6 sm:mb-8 gap-4">
              <div>
                <h3 className="text-2xl sm:text-3xl font-black uppercase tracking-tighter">
                  {editingTaskId ? 'Edit Event' : 'Log Event'}
                </h3>
                <p className={`text-[10px] font-black uppercase tracking-widest ${mutedText} mt-1`}>Date: {modalDate}</p>
              </div>
              <button onClick={closeModal} className={`${mutedText} hover:text-red-500 transition-colors`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  value={form.title}
                  onChange={e => setForm({ ...form, title: e.target.value })}
                  className={`w-full bg-transparent border-b-2 ${borderColor} py-2 text-lg sm:text-xl font-bold outline-none focus:border-yellow-400 transition-colors`}
                  placeholder="Meeting Title"
                />
              </div>
              ...
              <button
                type="submit"
                className="w-full py-4 sm:py-5 rounded-full font-black uppercase tracking-[0.2em] text-xs sm:text-sm mt-4 hover:opacity-90 active:scale-95 transition-all"
                style={{ backgroundColor: isDark ? 'white' : 'black', color: isDark ? 'black' : 'white' }}
              >
                {editingTaskId ? 'Update Sync' : 'Log Sync'}
              </button>
      ...
      {confirmState.open && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 backdrop-blur-xl bg-black/40">
          <div className={`${modalBg} w-full max-w-sm rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 shadow-2xl border ${borderColor} animate-in fade-in zoom-in duration-200`}>
            <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tighter mb-6 text-center">
              {confirmState.mode === 'delete' ? 'Confirm Delete?' : 'Save Changes?'}
            </h3>
            <div className="flex gap-3 sm:gap-4">
              <button
                onClick={closeConfirmModal}
                className={`flex-1 py-3 sm:py-4 rounded-full border-2 ${borderColor} text-[10px] font-black uppercase tracking-widest hover:opacity-70 transition-all`}
              >
                Cancel
              </button>
              <button
                onClick={handleConfirmAction}
                className="flex-1 py-3 sm:py-4 rounded-full text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-all"
                style={
      ...
      <aside className={`w-full xl:w-80 border-b xl:border-b-0 xl:border-r ${borderColor} p-4 sm:p-6 lg:p-10 space-y-6 sm:space-y-8 lg:space-y-12 flex-none ${sidebarBg} transition-colors duration-500 relative z-30 overflow-visible`}>
        <div className="space-y-2">
          <h2 className="text-xl sm:text-2xl lg:text-3xl font-black uppercase tracking-[0.1em]">Next Up</h2>
          <div className="h-1 w-12 bg-current opacity-20 rounded-full" />
        </div>
        ...
        <div className="space-y-4 sm:space-y-6">
          {selectedDayTasks.slice(0, 3).map(task => (
            <div
              ...
              className={`p-4 sm:p-5 lg:p-6 rounded-[1.5rem] sm:rounded-[2rem] border ${borderColor} transition-all relative overflow-visible group hover:scale-[1.02] cursor-pointer ${isDark ? 'bg-zinc-900/40' : 'bg-white shadow-sm'}`}
            >
              <div className="flex items-center gap-3 mb-4">
                <span className={`w-3 h-3 rounded-full ${task.priority === 'high' ? 'bg-red-600 animate-pulse' : 'bg-blue-600'}`} />
                <span className={`text-[10px] sm:text-xs font-black uppercase tracking-widest ${mutedText}`}>
                  {task.time} {task.dueDate === selectedDateKey ? 'Selected Day' : 'Scheduled'}
                </span>
              </div>
              <h4 className="font-black text-lg sm:text-xl lg:text-2xl mb-1 tracking-tight">{task.title}</h4>
              <p className={`text-[11px] sm:text-xs ${mutedText} font-bold leading-relaxed truncate`}>{task.description}</p>

              <div className="flex gap-2 mt-4 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                <button
                  onClick={e => {
                    e.stopPropagation();
                    openEditModal(task);
      ...
      <div className={`flex-1 p-4 sm:p-6 md:p-10 xl:p-14 space-y-8 sm:space-y-10 lg:space-y-12 overflow-y-auto ${bgColor} transition-colors duration-500 relative z-10`}>
        <header className="flex flex-col gap-6 lg:gap-8">
          <div className="space-y-3 sm:space-y-4">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div className="flex flex-col sm:flex-row sm:items-end gap-3 sm:gap-4">
                <h1 className="text-3xl sm:text-5xl md:text-7xl xl:text-8xl font-black tracking-tighter leading-none">
                  {monthName}
                  <span className={`block sm:inline sm:ml-4 ${isDark ? 'text-zinc-700' : 'text-zinc-200'}`}>{currentYear}</span>
                </h1>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => shiftMonth(-1)}
                    className={`p-2.5 sm:p-3 rounded-full border ${borderColor} hover:bg-current hover:text-current-contrast transition-all`}
            ...
            <p className={`text-[10px] sm:text-[11px] font-black uppercase tracking-[0.3em] sm:tracking-[0.5em] ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`}>
              Precision Synchronization View
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-5">
            <div className="relative group">
              <button
                ...
                className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 border-2 ${borderColor} rounded-full flex items-center justify-center transition-all group-hover:border-blue-500 group-hover:text-blue-500`}
              >
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <button
                onClick={() => shiftDay(-1)}
                className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 border-2 ${heavyBorderColor} rounded-full flex items-center justify-center transition-all hover:border-yellow-400 hover:text-yellow-400`}
                title="Yesterday"
              >
                <svg className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path d="M15 19l-7-7 7-7" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
              ...
              <button
                onClick={goToToday}
                className={`px-5 sm:px-8 md:px-12 h-12 sm:h-14 md:h-16 border-2 ${heavyBorderColor} rounded-full font-black uppercase text-[10px] sm:text-xs md:text-sm tracking-[0.15em] sm:tracking-[0.2em] transition-all hover:border-yellow-400 hover:text-yellow-400 shadow-sm active:scale-95`}
              >
                Today
              </button>
              <button
        ...
        <div className="grid grid-cols-7 gap-2 sm:gap-3 md:gap-4">
          {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map(day => (
            <div
              key={day}
              className={`text-center text-[9px] sm:text-[11px] font-black uppercase tracking-[0.2em] sm:tracking-[0.4em] pb-3 sm:pb-5 md:pb-8 ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`}
            >
              {day}
            </div>
          ))}
          ...
          {calendarData.map((day, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`min-h-[96px] sm:min-h-[140px] md:min-h-[180px] xl:min-h-[220px] p-2 sm:p-4 md:p-6 xl:p-8 rounded-[1.25rem] sm:rounded-[2rem] xl:rounded-[3rem] border-2 transition-all flex flex-col relative overflow-hidden group ${
                  day
                    ? isActualToday
                      ? isDark
                ...
                {day && (
                  <>
                    <button
                      ...
                      className={`absolute bottom-2 right-2 sm:bottom-4 sm:right-4 xl:bottom-8 xl:right-8 w-8 h-8 sm:w-10 sm:h-10 xl:w-14 xl:h-14 rounded-full border-2 flex items-center justify-center text-xl sm:text-2xl xl:text-3xl font-black transition-all hover:scale-110 active:scale-90 ${
                        isActualToday
                          ? isDark
                          ...
                          : isSelected
                            ...
                            : 'border-current opacity-40 sm:opacity-30 group-hover:opacity-100 hover:bg-current/10'
                      }`}
                      title="Log Meeting/Event"
                    >
                      +
                    ...
                    <span
                      className={`text-2xl sm:text-4xl md:text-6xl xl:text-7xl font-black leading-none tracking-tighter mb-2 sm:mb-3 md:mb-4 transition-colors ${
                        isActualToday ? (isDark ? 'text-black' : 'text-white') : isSelected ? 'text-yellow-400' : 'group-hover:text-yellow-400'
                      }`}
                    >
                      {day}
                    ...
                    <div className="space-y-1 sm:space-y-2 mt-auto pr-8 sm:pr-12 xl:pr-16">
                      {dayTasks.slice(0, 2).map(task => (
                        ...
                        >
                          <div
                            className={`text-[8px] sm:text-[10px] px-2 sm:px-4 py-1.5 sm:py-2 rounded-full font-black uppercase tracking-[0.1em] sm:tracking-widest truncate cursor-pointer ${
                              isActualToday
                                ? isDark
                                  ? 'bg-black text-white'
                                  : 'bg-white text-black'
                          ...
                          <div className="flex gap-2 mt-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              ...
                              className="text-[8px] font-black uppercase tracking-tighter text-red-500 opacity-70 hover:opacity-100"
                            >
                              Delete
                            </button>
                          </div>
      ...
      {previewTask &&
        ...
        createPortal(
          <div
            className={`fixed z-[9999] rounded-[1.5rem] sm:rounded-[2rem] p-5 sm:p-6 border ${borderColor} shadow-2xl ${modalBg} pointer-events-none`}
            style={{ top: previewPos.top, left: previewPos.left, width: 'min(340px, calc(100vw - 24px))' }}
          >
            <h5 className={`text-base sm:text-lg font-black uppercase tracking-tight ${isDark ? 'text-white' : 'text-black'}`}>
              {previewTask.title}
            </h5>

            <p className="text-[12px] sm:text-[13px] font-black tracking-widest text-yellow-400 mt-1">
              {previewTask.time || 'N/A'}
            </p>

            <div className="space-y-2 text-[10px] sm:text-[11px] font-bold mt-4">
              <div className={mutedText}>{previewTask.description || 'N/A'}</div>
              <div className={mutedText}>{previewTask.location || 'N/A'}</div>
            </div>
          </div>,
