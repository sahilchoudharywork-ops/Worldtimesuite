...
let countryIndexLoadPromise: Promise<void> | null = null;

const loadCountryZoneIndex = (): Promise<void> => {
  if (Object.keys(COUNTRY_ZONE_INDEX).length > 0) return Promise.resolve();
  if (countryIndexLoadPromise) return countryIndexLoadPromise;

  countryIndexLoadPromise = import('countries-and-timezones')
    .then(ctModule => {
      const countries = ctModule.getAllCountries();
    ...
    .catch(() => {
      // Alias and IANA matching still cover common city/timezone searches.
    });
...
const TimezoneConverter: React.FC<TimezoneConverterProps> = ({ isDark, fromSlug, toSlug, isTimezoneCodeRoute = false }) => {
  ...
  useEffect(() => {
    ...
    const applyRoute = () => {
      ...
      return Boolean(routeSource && routeTarget);
    };

    const resolvedImmediately = applyRoute();

    if (!resolvedImmediately) {
      void loadCountryZoneIndex().then(() => {
        if (!cancelled) applyRoute();
      });
    }
  ...
  const getTimelineData = useCallback((iana: string, startUtc: Date) => {
    const cells = [];
  ...
  const allTzs = useMemo(() => [sourceTz, ...targets], [sourceTz, targets]);

  const timelineRows = useMemo(
    () => allTzs.map(tz => getTimelineData(tz.iana, timelineStartUtc)),
    [allTzs, getTimelineData, timelineStartUtc],
  );
  ...
  return (
    <div className={`timezone-no-shadow p-4 sm:p-8 space-y-8 sm:space-y-12 ${bgColor} ${textColor} font-['Helvetica']`}>
      ...
      <div className="max-w-4xl mx-auto flex flex-col sm:flex-row gap-4">
        <div className={`flex-grow flex items-center px-4 sm:px-5 py-3 sm:py-2 rounded-full border-2 ${borderClass} focus-within:border-blue-500 transition-all shadow-2xl ${inputBg}`}>
          ...
          <input
            ...
            value={naturalInput}
            onFocus={() => { void loadCountryZoneIndex(); }}
            onChange={e => {
              setNaturalInput(e.target.value);
              if (e.target.value.length > 1) {
                void loadCountryZoneIndex();
                performLocalParse(e.target.value, false);
              }
            }}
      ...
      {showDeferredSections && showRelatedRoutes && (
            ...
            <div className="flex flex-col lg:flex-row">
              ...
              <div className="flex-grow overflow-x-auto scrollbar-custom">
                <div className="min-w-max">
                  {allTzs.map((tz, rowIndex) => {
                    const timeline = timelineRows[rowIndex] || [];
                    return (
