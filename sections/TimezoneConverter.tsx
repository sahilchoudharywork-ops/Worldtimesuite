...
//TESTING CODE 3 - Adding a new tile for "More time conversions
...
//     const addRoute = (href: string, label: string) => {
//       if (routes.length >= 8) return;
//       if (href === `/${currentRoute}`) return;
//       if (routes.some(r => r.href === href)) return;
//       routes.push({ href, label });
//     };

//     (fromRegion ? REGION_CLUSTERS[fromRegion] || [] : []).slice(0, 4).forEach(city => {
//       if (city === fromRouteSlug || city === toRouteSlug) return;
//       addRoute(`/${fromRouteSlug}-to-${city}`, `${fromCityName} → ${humanizeSlug(city)}`);
//     });

//     (toRegion ? REGION_CLUSTERS[toRegion] || [] : []).slice(0, 4).forEach(city => {
//       if (city === fromRouteSlug || city === toRouteSlug) return;
//       addRoute(`/${city}-to-${toRouteSlug}`, `${humanizeSlug(city)} → ${toCityName}`);
//     });

//     return routes;
//   }, [fromRouteSlug, toRouteSlug, currentRoute, fromCityName, toCityName]);
...
//       {relatedRoutes.slice(0, 4).map(route => (
//         <a
//           key={route.href}
//           href={route.href}
//           className={`
...
//TESTING CODE 4 - Smart Related routes generation based on city clusters and regions 
